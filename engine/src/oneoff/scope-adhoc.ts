/**
 * ONE-OFF (ad hoc): XNDR — Scope of Works in the "engineering table" style of the
 * source tender (scope-example.pdf). A4 LANDSCAPE, fully ruled: a bordered item
 * table (ITEM bar → LOCATION / SCOPE INTENT / EXTENT / SPECIFICATION rows), a
 * two-column specification body, and a gridded title-block footer strip — all in
 * XNDR branding, in light and dark. Tuned for a tight, compact read. Not wired
 * into the CLI. Run:
 *   cd engine && npx tsx src/oneoff/scope-adhoc.ts
 * Reuses examples/sample-scope.json as its data. Output → git-ignored designs/
 * as adhoc-scope-light.pdf / adhoc-scope-dark.pdf.
 */
...
function itemSection(item: ScopeItem): string {
  ...
  return `<section class="item">
    ...
    ${rows.join("")}
    ${body ? `<div class="drow"><div class="lbl">Scope of Works &amp; Specification</div><div class="val spec"><div class="cols">${body}</div></div></div>` : ""}
  </section>`;
}
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cov-hc.right { flex: 1; }
  .cov-body { display: flex; height: 118mm; }
  ...
  /* ── Item table ──────────────────────────────────────────────────── */
  .item { border: 0.75pt solid ${rule}; border-top: none; }
  ...
  .item > .ihead + .drow { border-top: none; }
  .lbl {
  ...
  /* Two-column specification body — the source's signature layout. */
  .cols { columns: 2; column-gap: 8mm; column-rule: 0.5pt solid ${rule}; }
  .block { break-inside: avoid; margin-bottom: 2mm; }
  .block:last-child { margin-bottom: 0; }
  .block-h { font-weight: 700; text-decoration: underline; font-size: 7.4pt; margin: 1.3mm 0 1mm; }
...
async function main(): Promise<void> {
  ...
  for (const theme of ["light", "dark"] as ThemeName[]) {
    ...
    const out = resolve(DESIGNS_DIR, `adhoc-scope-${theme}.pdf`);
    await renderHtmlToPdf(html, out);
    console.log(`✓ Ad-hoc Scope of Works (${theme}) → ${out}`);
  }
}
